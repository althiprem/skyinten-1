...
export default function AuthModal({ isOpen, type, onClose, onSuccess }) {
  ...
  return (
    <div
      className={`authModal__overlay ${isOpen && !closing ? "show" : ""} ${closing ? "closing" : ""
        }`}
      onClick={handleClose}
    >
      <div
        className={`authModal__content ${isOpen && !closing ? "show" : ""} ${closing ? "closing" : ""
          }`}
      ...
      >
        ...
        <div className={`authModal__inner ${switching ? "switching" : ""}`}>
          ...
          <form action="" method="post" className="authModal__form" onSubmit={handleEmailAuth}>
            {localType === "signup" && (
